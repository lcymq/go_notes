...
func main() {
	...
	fmt.Println(p.Name, p.Age) // 墨香 30
}
```
↑ 如果用非`person`类的数据类型，什么都读取不出来。但要是有`name`和`age`字段的话，可以读取并赋值，比如下面这个结构体就可以：
```go
type student struct {
	Name string `json:"name"`
	Age  int    `json:"age"`
...
##### type name和type kind
在反射中关于类型还划分为两种：`类型（Type）`和`种类（Kind）`。因为在Go语言中我们可以使用type关键字构造很多自定义类型，而种类（Kind）就是指底层的类型，但在反射中，当需要区分指针、结构体等大品种的类型时，就会用到种类（Kind）。 举个例子，我们定义了两个指针类型和两个结构体类型，通过反射查看它们的类型和种类。
